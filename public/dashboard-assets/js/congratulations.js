...
// Show Success Message After Page Reload
let $ajaxform_instant_reload_after_confirm = $(
    ".ajaxform_instant_reload_after_confirm"
);
...
$(document).on(
    ...
    function (e) {
        ...
        if ($ajaxform_instant_reload_after_confirm.valid()) {
            let icon =
                $(this).find(".submit-btn").data("icon") ?? "fas fa-warning";
...
function NotifyAlert(type, res, msg = null, destination = null) {
    ...
    switch (type) {
        case "error":
            background =
                "radial-gradient(circle at 10% 50.5%, rgb(255, 107, 6) 0%, rgb(255, 1, 107) 90%)";
            message =
                msg ??
                res.message ??
                res.responseText ??
        ...
        case "success":
            ...
            message =
                msg ?? res.message ?? "Congratulate! Operation Successful.";
            destination = destination ?? redirect ?? null;
            break;
        case "warning":
            background =
